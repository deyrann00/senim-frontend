...
export default function AboutPage({ t }) {
  const at = t?.about || {}; // Аудармаларды жүктеу
      ...
      <div>
        <section style={{ background: COLORS.blue, padding: "4rem 1.25rem 3rem", textAlign: "center", color: "white" }}>
          ...
          <h1 style={{ fontSize: "2.2rem", fontWeight: 700, marginBottom: "1rem" }}>
            {at?.heroTitle || "Қазақстанды киберқылмысқа қарсы күшейту"}
          </h1>
          <p style={{ opacity: 0.8, fontSize: 16, maxWidth: 520, margin: "0 auto" }}>
            {at?.heroSub || "SenimWiki — халықты оқытуға және қазақстандықтарды қаржылық алаяқтықтан қорғауға арналған тегін платформа."}
          </p>
        </section>
              ...
              {!sent ? (
                  <div style={{ display: "flex", flexDirection: "column", gap: "0.85rem" }}>
                    ...
                    <button
                        ...
                        style={{ background: COLORS.blue, color: "white", border: "none", borderRadius: 10, padding: "0.75rem", fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}
                    >
                      {at?.sendBtn || "Жіберу"}
                    </button>
                  </div>
                  ...
                  <div style={{ background: COLORS.greenLight, border: `1.5px solid ${COLORS.green}`, borderRadius: 12, padding: "1.5rem", textAlign: "center" }}>
                    ...
                    <p style={{ fontWeight: 600, color: "#065f46", margin: 0 }}>
                      {at?.sentMsg || "Хабарлама жіберілді! 24 сағат ішінде жауап береміз."}
                    </p>
                  </div>
              )}
            </div>
            <div>
              ...
              <div style={{ background: COLORS.amberLight, border: `1px solid #fcd34d`, borderRadius: 12, padding: "1rem 1.25rem" }}>
                <p style={{ color: "#92400e", fontSize: 13, margin: 0, lineHeight: 1.6 }}>
                  <strong>{at?.urgentLabel || "Шұғыл жағдай?"}</strong> {at?.urgentText || "Егер сіз алаяқтардың құрбаны болсаңыз, Ұлттық қаржылық қауіпсіздік желісіне хабарласыңыз:"} <strong>1400</strong>
                </p>
              </div>
            </div>
          </div>
