...
import { Shield, BookOpen, Award, Database, Info, Globe, Menu, X, LayoutDashboard, Lock } from 'lucide-react';
import { C } from '../config/constants';

export function Navbar({ page, setPage, lang, setLang, t, safetyScore, isAdmin }) {
  ...
  // ЕГЕР АДМИН БОЛСА, НАВБАРҒА АДМИН ПАНЕЛЬ ТАБЫН ҚОСАМЫЗ
  if (isAdmin) {
    links.push({ id: "admin", label: "Admin", icon: <LayoutDashboard size={15}/> });
  }
...
export function Footer({ t, setPage }) {
  const ft = t.footer;
  const [isMobile, setIsMobile] = useState(false);
  ...
  return (
      <footer style={{ background:C.blue, color:"rgba(255,255,255,0.7)", marginTop:"auto" }}>
        ...
        <div style={{ borderTop:"1px solid rgba(255,255,255,0.1)", padding:"1rem", display: "flex", justifyContent: "space-between", alignItems: "center", fontSize:12 }}>
          <span>{ft.copyright}</span>
          {/* ЖАСЫРЫН АДМИН ТҮЙМЕСІ */}
          <button onClick={() => setPage("login")} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.3)", cursor: "pointer", padding: "5px" }}>
            <Lock size={12} />
          </button>
        </div>
      </footer>
